...
class Conference_images extends CI_Controller {
	...
	{	
		if($this->session->userdata('userdetails'))
		{
			$this->load->view('admin/conference-images/add');
	...
	{	
		if($this->session->userdata('userdetails'))
		{
			...
			//echo '<pre>';print_r($data);exit; 
			$this->load->view('admin/conference-images/list',$data);
			$this->load->view('admin/footer');
		}else{
			$this->session->set_flashdata('error','Please login to continue');
			redirect('admin');
	...
	{	
		if($this->session->userdata('userdetails'))
		{
			...
			//echo '<pre>';print_r($data);exit; 
			$this->load->view('admin/conference-images/edit',$data);
			$this->load->view('admin/footer');
		}else{
			$this->session->set_flashdata('error','Please login to continue');
			redirect('admin');
	...
	{	
		if($this->session->userdata('userdetails'))
		{
			...
			//echo '<pre>';print_r($post);exit;
					...
					$save=$this->Conference_img_model->save_flyers($add_data);
						if(count($save)>0){
							$this->session->set_flashdata('success','Conference image successfully Added');
							redirect('conference-images/lists');
							
						}else{
							$this->session->set_flashdata('error',"technical problem will occurred. Please try again.");
							redirect('conference-images');
						}
		}else{
			$this->session->set_flashdata('error','Please login to continue');
			redirect('admin');
