...
class Banners extends CI_Controller {
	...
	{	
		if($this->session->userdata('userdetails'))
		{
			$this->load->view('admin/index_banners/add');
	...
	{	
		if($this->session->userdata('userdetails'))
		{
			...
			//echo '<pre>';print_r($data);exit; 
			$this->load->view('admin/index_banners/list',$data);
			$this->load->view('admin/footer');
		}else{
			$this->session->set_flashdata('error','Please login to continue');
			redirect('admin');
	...
	{	
		if($this->session->userdata('userdetails'))
		{
			...
			//echo '<pre>';print_r($data);exit; 
			$this->load->view('admin/index_banners/edit',$data);
			$this->load->view('admin/footer');
		}else{
			$this->session->set_flashdata('error','Please login to continue');
			redirect('admin');
		...
		if($this->session->userdata('userdetails'))
		{
			...
			//echo '<pre>';print_r($post);exit;
						if(isset($_FILES['image']['name']) && $_FILES['image']['name']!=''){
								...
								move_uploaded_file($_FILES['image']['tmp_name'], "assets/index_banners/" . $image);
							}else{
								$image='';
							}
					$add_data=array(
					...
					$save=$this->Banners_model->save_banners($add_data);
						if(count($save)>0){
							...
							redirect('index_images/lists');
							
						}else{
							$this->session->set_flashdata('error',"technical problem will occurred. Please try again.");
							redirect('index_images');
						}
		}else{
			$this->session->set_flashdata('error','Please login to continue');
			redirect('admin');
	...
	{	
		if($this->session->userdata('userdetails'))
		{
			...
			$details=$this->Banners_model->get_banners_details($f_id);
					...
					if(count($delete)>0){
						unlink('assets/index_banners/'.$details['image']);
